...
class Provider {
    ...
    async findEpisodes(id: string): Promise<EpisodeDetails[]> {
        const res = await fetch(`https://jkanime.net/${id}`);
        ...
        const results = episodes.map(([episodeNum]) => ({
            ...
            number: episodeNum,
            url: `https://jkanime.net/${id}/${episodeNum}`,
    ...
    async findEpisodeServer(episode: EpisodeDetails, _server: string): Promise<EpisodeServer> {
        ...
        const headers = {
            ...
            "Range": "bytes=0-",
            "Referer": "https://jkanime.net/jkplayer/c1?u=",
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/139.0.0.0 Safari/537.36",
        };

        // Streamtape
        const StreamtapeRes = await fetch(selected.url);
        const StreamtapeHtml = await StreamtapeRes.text();


        let videoMatch = streamtapeHtml.match(/<video[^>]+src=["']([^"']+)["']/i);
        let usedFallback = false;

        if (!videoMatch) {
            videoMatch = streamtapeHtml.match(/file:\s*['"]([^'"]+\.mp4)['"]/i);
